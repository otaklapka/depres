...
use crate::k8s::commons::{Metadata, PrintResources, map_to_table_value};
...
impl StatefulSet {
    pub fn requests_storage(&self, multiplier: Option<u32>) -> Option<f64> {
        let requests_storage_sum = self.spec.volume_claim_templates.as_ref()?.iter().map(|pvc | pvc.requests_storage()).sum::<Option<f64>>()?;
    ...
    pub fn limits_storage(&self, multiplier: Option<u32>) -> Option<f64> {
        let limits_storage_sum = self.spec.volume_claim_templates.as_ref()?.iter().map(|pvc | pvc.limits_storage()).sum::<Option<f64>>()?;
